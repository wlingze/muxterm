...
run_macos() {
    ...
    if [[ "$(uname -s)" == "Darwin" ]]; then
        mkdir -p src/frontend/macos/Vendor
        ln -sfn "$PWD/target/release/libmuxterm.a" src/frontend/macos/Vendor/libmuxterm.a
        (cd src/frontend/macos && ../../../scripts/patch-swiftterm.sh)
        # 先构建 AppKit executable，再跑 headless tests；否则 SettingsWindow 等
        # app-only 源文件的语法错误可能被 swift test 的测试目标选择漏掉。
        (cd src/frontend/macos && swift build)
