...
/// 数一批事件里的 PaneOutput。
#[allow(dead_code)]
pub fn count_pane_output_events(
    events: &[muxterm::test_support::core::protocol::state::StateChange],
) -> usize {
    events
        .iter()
        .filter(|e| {
            matches!(
                e,
                muxterm::test_support::core::protocol::state::StateChange::PaneOutput { .. }
            )
        })
        .count()
}
