...
第一波同时消化和「谁在等我」绑在一起的项：机器标记、agent 键与 title。
...
| 回底 | 离开几行才显示胶囊；贴尾才藏；**迟滞、不闪**；agent TUI 默认不吸附、可不显示胶囊 |
...
- 粘贴文本仍走现在的系统剪贴板覆盖语义，和图片投递是两条路：剪贴板是图 → 投递；是字 → 当字贴。
...
| 波 | 收口范围 |
| --- | --- |
| 1 B | 机器标记、agent 键/title/OSC |
| 2 地标/搜索 | 刻度与回底、误跳、拖选、胶囊闪 |
| 3 日用 | 工作区导航、Cmd-W、选词 |
| 4 远程 | 图片走愿景 §4.2；端口转发是本轮新加 |

重构落地后：**先把已有行为迁到新树，再按本文件的四波往 1.0 收。** 不要在旧抽象上把 B 和端口转发做完再 rebase。
