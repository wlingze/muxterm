...
## 27. 再往下想：身份、几条新想法、进一步做什么（2026-09-09）

不是新开一条功能清单。功能已经够 1.0。缺的是 **想清楚这扇窗是干什么的**，好挡住以后想加的东西。

### 身份（用来否决，不是用来写 README）

Muxterm 是 **望向已有工作的那扇原生窗**。工作活在 tmux / Herdr / 将来的 Superlogical 里。窗负责：手该放哪、一眼谁在等、随手敲、远程的图和端口别逼人再开一个 App。

- Herdr 把「谁在等我」做进 **Runtime + TUI**。Muxterm 做进 **Tab/窗/面板**，并且跨 tmux 与 Herdr。
- Superlogical 换 **管道**。Muxterm 不换管道，管道开了再接。
- 一做成 agent 看板、第二套 mux、账号，身份就没了。
...
没有新的第六条产品线。有新想法先问：是窗上的聚合/状态/投递，还是在造 Runtime？后者不做。
