...
impl Workspace {
    ...
    pub fn pane_command_marks(
        &self,
        pane: PaneId,
    ) -> Vec<crate::protocol::terminal::emulate::CommandMark> {
        self.panes
            .get(&pane)
    ...
    pub fn pane_previous_command_mark(
        ...
        pane: PaneId,
        current_seq: u64,
    ) -> Option<crate::protocol::terminal::emulate::CommandMark> {
        self.panes
            .get(&pane)
...
mod tests {
    ...
    fn new_tab_without_workdir_uses_workspace_project_path() {
        ...
        let mut spec = WorkspaceSpec::local_tmux(Some("demo".into()), None);
        spec.path = "/tmp/a".into();
    ...
    #[test]
    fn new_tab_without_resolved_target_uses_workspace_id_path() {
        use std::sync::{Arc, Mutex};

        let log = Arc::new(Mutex::new(Vec::new()));
        let mut runtime = MockRuntime::with_single_pane();
        runtime.executed_log = Some(log.clone());
        let id = WorkspaceId::new("local", None, "demo", "tmux", "/tmp/from-id");
        let mut workspace = Workspace::new(id, "demo".into(), Box::new(runtime));
        workspace
            .execute(Task::NewTab {
                name: None,
    ...
    fn byte_ring_drops_oldest_but_search_finds_recent() {
        ...
        let mut old = Vec::new();
        while old.len() < crate::buffer_cap::MAX_PANE_OUTPUT_BYTES + 1024 {
        ...
        let raw = w.pane_raw_bytes(PaneId(1));
        assert!(
            raw.len() <= crate::buffer_cap::MAX_PANE_OUTPUT_BYTES,
            "byte ring 应有界: {}",
            raw.len()
        );
        assert!(
        ...
        let template = WorkspaceTemplate {
            ...
            tabs: vec![TabTemplate {
                ...
                layout: TemplateLayout::Split {
                    dir: crate::protocol::layout::SplitDir::Horizontal,
