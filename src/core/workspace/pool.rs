...
impl WorkspacePool {
    ...
    pub async fn open_spec(
        &mut self,
        spec: &crate::workspace::spec::WorkspaceSpec,
        create: impl FnOnce(&crate::workspace::spec::WorkspaceSpec) -> anyhow::Result<Box<dyn Runtime>>,
    ) -> anyhow::Result<&mut Workspace> {
        let id = spec.id();
        let name = spec.name();
    ...
    pub fn search_all(&self, query: &str) -> Vec<crate::workspace::workspace::SearchHit> {
...
mod tests {
    ...
    async fn reopen_spec_reuses_without_runtime_factory() {
        let mut pool = WorkspacePool::new(WorkspacePoolPolicy::new(4));
        let spec = crate::workspace::spec::WorkspaceSpec::local_shell("");
