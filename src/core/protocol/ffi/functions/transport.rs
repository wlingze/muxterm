...
pub extern "C" fn muxterm_discover_ssh_hosts_json(config_path: *const c_char) -> *mut c_char {
    catch_unwind(AssertUnwindSafe(|| {
        ...
        match crate::discovery::list_ssh_hosts(path.as_deref().map(std::path::Path::new)) {
    ...
    catch_unwind(AssertUnwindSafe(|| {
        ...
        let result = match transport.as_str() {
            "local" => {
                ...
                Ok(crate::discovery::list_local_dir(std::path::Path::new(
                    &expanded,
                )))
            }
            "ssh" => {
                ...
                crate::discovery::list_remote_dir(
                    alias,
                    &path,
...
pub extern "C" fn muxterm_discover_tmux_sessions_json(
    ...
    catch_unwind(AssertUnwindSafe(|| {
        ...
        let result = match transport.as_str() {
            "local" => Ok(crate::discovery::list_local_tmux_sessions(
                socket.as_deref(),
            )),
            "ssh" => crate::discovery::list_ssh_tmux_sessions(
                &target,
                config_path.as_deref(),
                socket.as_deref(),
                timeout,
