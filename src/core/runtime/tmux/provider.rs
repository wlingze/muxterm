...
impl RuntimeProvider for TmuxDriver {
    ...
    fn discover(
        ...
        self.discover_on_socket(connect, None)
    }
    ...
    fn discover_scoped(
        ...
        self.discover_on_socket(connect, spec.socket.as_deref())
    }
...
impl TmuxDriver {
    fn discover_on_socket(
        ...
        let (sessions, socket) = if connect.transport_id() == "ssh" {
            ...
            let remote_socket = socket
                .map(str::to_string)
                .or_else(|| std::env::var("MUXTERM_TEST_REMOTE_TMUX_SOCKET").ok());
            ...
            let local_socket = socket
                .map(str::to_string)
                .or_else(|| std::env::var("MUXTERM_TEST_LOCAL_TMUX_SOCKET").ok());
