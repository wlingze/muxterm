...
impl HerdrRuntime {
    ...
    pub fn worktrees(&self) -> anyhow::Result<Vec<crate::runtime::WorktreeInfo>> {
        let list = self.session.worktree_list(&self.workspace_id)?;
        Ok(list
            .worktrees
            .into_iter()
            .map(|w| crate::runtime::WorktreeInfo {
                path: w.path,
                branch: w.branch,
    ...
    fn key_to_herdr(key: &KeyEvent) -> String {
        match key {
            ...
            KeyEvent::Arrow(dir) => match dir {
                crate::protocol::terminal::input::ArrowDir::Up => "up".into(),
                crate::protocol::terminal::input::ArrowDir::Down => "down".into(),
                crate::protocol::terminal::input::ArrowDir::Left => "left".into(),
                crate::protocol::terminal::input::ArrowDir::Right => "right".into(),
            },
        }
    }
...
mod tests {
    use super::*;
    use crate::runtime::herdr::session::{
    ...
    #[test]
    fn new_tab_mutation_keeps_explicit_workdir() {
        let mut runtime = HerdrRuntime::new(
            Arc::new(HerdrSession::new("test", "/tmp/muxterm-no-socket")),
            "w1",
        );
    ...
    fn pane_read_seeds_index_without_surface_event() {
        ...
        let queued_events = queued_state_changes(&runtime);
        let surface_events = queued_events
            .iter()
            .filter(|event| {
                matches!(
                    event,
