...
use crate::linux::attention_ui::window_title;
use crate::linux::quickconnect::status_style::StatusBarSnapshot;
use crate::linux::status_bar::ConnectionSummary;
...
pub(super) fn refresh_connection_summary(s: &mut UiState) {
    ...
    let (down_rate, up_rate) = match (s.last_traffic, s.last_traffic_at) {
        (Some((pdown, pup)), Some(at)) => {
            ...
            (
                crate::format::rate_bps(pdown, down, dt),
                crate::format::rate_bps(pup, up, dt),
            )
        }
        _ => (0, 0),
    };
...
pub(super) fn local_status_snapshot(
    ...
    let mut snap =
        crate::linux::quickconnect::status_style::snapshot_from_tabs("local", npanes, tabs);
...
pub(super) fn maybe_refresh_status(s: &mut UiState, force: bool) {
    ...
    let mut snap = if s.uses_tmux() {
        crate::linux::quickconnect::status_style::snapshot_from_tabs(&session, npanes, &rows)
    } else {
