...
use crate::linux::lifecycle::cycle_pane_id;
...
pub(super) fn open_command_palette(s: &UiState, window: &Window, state: &Rc<RefCell<UiState>>) {
    ...
    crate::linux::command_palette::show_for_runtime(
        &parent,
        uses_tmux,
        next_theme,
        next_status_mode,
...
pub(super) fn run_palette_command(
    ...
    match action {
        PaletteAction::Language => {
            ...
            let callback_state = state.clone();
            crate::linux::command_palette::show_language(&language_parent, move |_| {
...
pub(super) fn paste_pane(s: &UiState, state: &Rc<RefCell<UiState>>, pane_id: u32) {
    ...
    clipboard.read_text_async(gtk4::gio::Cancellable::NONE, move |result| {
        ...
        let text = crate::mirror::sanitize_paste(text.as_str(), bracketed);
        let data = crate::mirror::encode_clipboard_paste(&text, bracketed);
        if data.is_empty() {
            return;
        }
