...
impl AppWindow {
    ...
    pub fn test_layout_leaf_ids(&self) -> Vec<u32> {
        ...
        fn leaves(layout: &crate::ffi_client::ClientLayout, out: &mut Vec<u32>) {
            match layout {
                crate::ffi_client::ClientLayout::Leaf { pane_id } => out.push(*pane_id),
                crate::ffi_client::ClientLayout::Split { first, second, .. } => {
                    leaves(first, out);
                    leaves(second, out);
                }
            }
    ...
    pub fn test_inject_fault(&self, token: &str) {
        crate::linux::fault_gtk::inject_fault(token);
    }
    ...
    pub fn test_attention_snapshot(&self) -> crate::ffi_client::ClientActivitySnapshot {
