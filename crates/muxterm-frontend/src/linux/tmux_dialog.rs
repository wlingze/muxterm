...
use crate::ffi_client::{ExistingCandidate, FfiClient, SshHostEntry};
use crate::linux::pane_switcher;
use crate::linux::quick_pick::{self, QuickPickItem};
...
pub fn tmux_session_pick_items(sessions: &[ExistingCandidate]) -> Vec<QuickPickItem> {
    ...
    items.push(QuickPickItem {
        id: CREATE_ID.into(),
        label: crate::i18n::tr(crate::i18n::Key::TmuxCreateNew),
        detail: Some(crate::i18n::tr(crate::i18n::Key::TmuxCreateDetail)),
    });
    for s in sessions {
        let attached = if s.attached {
            format!(" · {}", crate::i18n::tr(crate::i18n::Key::TmuxAttached))
        } else {
            String::new()
        };
        items.push(QuickPickItem {
    ...
    F: Fn(TmuxAction) + 'static,
{
    ...
    quick_pick::show(
        &parent_win,
        &crate::i18n::tr(crate::i18n::Key::TmuxAttachPlaceholder),
        items,
        move |picked| {
...
fn format_session_detail(s: &WorkspaceInfo) -> String {
    let age = s
        ...
        .unwrap_or_else(|| crate::i18n::tr(crate::i18n::Key::TmuxUnknown));
    let wins = s
        .windows
        .map(|n| n.to_string())
        .unwrap_or_else(|| crate::i18n::tr(crate::i18n::Key::TmuxUnknown));
    crate::i18n::tr_args(
        crate::i18n::Key::TmuxSessionDetail,
        &[("age", &age), ("count", &wins)],
    )
}
...
fn relative_age_label(created_secs: u64, now: u64) -> String {
    ...
    let (key, count) = if ago < 60 {
        (crate::i18n::Key::TmuxSecondsAgo, ago)
    } else {
        let mins = ago / 60;
        if mins < 60 {
            (crate::i18n::Key::TmuxMinutesAgo, mins)
        } else {
            let hours = mins / 60;
            if hours < 48 {
                (crate::i18n::Key::TmuxHoursAgo, hours)
            } else {
                (crate::i18n::Key::TmuxDaysAgo, hours / 24)
            }
        }
    };
    let count = count.to_string();
    crate::i18n::tr_args(key, &[("count", &count)])
}
...
mod tests {
    ...
    fn format_session_detail_contains_windows() {
        ...
        let age = crate::i18n::tr_args(crate::i18n::Key::TmuxHoursAgo, &[("count", "2")]);
