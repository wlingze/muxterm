    ...
    F: Fn(&str) + 'static,
{
    ...
    let placeholder = crate::i18n::tr(crate::i18n::Key::CommandPalettePlaceholder);
...
where
    F: Fn(crate::i18n::Language) + 'static,
{
    let current = crate::i18n::current_language();
    let items: Vec<QuickPickItem> = crate::i18n::Language::ALL
        .into_iter()
        .map(|language| QuickPickItem {
            id: language.tag().into(),
            label: match language {
                crate::i18n::Language::System => {
                    crate::i18n::tr_in(language, crate::i18n::Key::LanguageSystem)
                }
                crate::i18n::Language::English => {
                    crate::i18n::tr_in(language, crate::i18n::Key::LanguageEnglish)
                }
                crate::i18n::Language::SimplifiedChinese => {
                    crate::i18n::tr_in(language, crate::i18n::Key::LanguageSimplifiedChinese)
                }
            },
            detail: (current == language)
                .then(|| crate::i18n::tr(crate::i18n::Key::LanguageCurrent)),
        })
        .collect();
    quick_pick::show(
        parent,
        &crate::i18n::tr(crate::i18n::Key::Language),
        items,
        move |picked| {
            if let Some(item) = picked {
                let language = match item.id.as_str() {
                    "system" => crate::i18n::Language::System,
                    "zh-CN" => crate::i18n::Language::SimplifiedChinese,
