...
use super::{
    ...
    parse_workspace_id, workspace_replica_id, AgentSidebarItem, ClientTask, ExistingNav, PanelTab,
    UiState,
};
...
pub(super) fn open_panel(state: &Rc<RefCell<UiState>>, window: &Window, initial_tab: PanelTab) {
    ...
    crate::linux::quickconnect_panel::show(
        &win,
        crate::linux::quickconnect_panel::PanelShowArgs {
            initial_tab,
            workspaces,
            workspace_search_items,
            agents,
                ...
                std::boxed::Box::new(move |query, scope| {
                    ...
                    let hits = s
                        ...
                        .filter(|hit| match scope {
                            crate::linux::panel_model::SearchScope::Pane => {
                                hit.workspace_id == workspace_replica
                                    && hit.pane_id == s.active_pane
                            }
                            crate::linux::panel_model::SearchScope::Workspace => {
                                hit.workspace_id == workspace_replica
                            }
                            crate::linux::panel_model::SearchScope::All => true,
                        })
                        .map(crate::linux::panel_model::SearchRow::from)
                        .collect();
                    hits
                })
            },
...
pub(super) fn jump_to_attention_pane(state: &Rc<RefCell<UiState>>, ws: &str, pane: u32, seq: u64) {
    ...
    drop(s);
    crate::linux::quickconnect_panel::close_current();
}
