...
use crate::ffi_client::{ClientConfig, FfiClient};
use crate::linux::keymap::default_keybindings;
use crate::linux::theme::fallback_theme;
#[cfg(test)]
...
pub fn run(socket: Option<String>) -> anyhow::Result<()> {
    ...
    if let Err(error) = crate::linux::font_registry::register_bundled_fonts() {
        tracing::warn!(
            target = "muxterm::app",
    ...
    app.connect_activate(move |a| {
        ...
        let win = crate::linux::window::AppWindow::new_with_effective_keybindings(
            cfg,
            theme,
            &keybindings,
        );
