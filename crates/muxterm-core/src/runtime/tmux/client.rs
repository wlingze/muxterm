...
const MAX_RESPONSE_BYTES: usize = crate::buffer_cap::MAX_PANE_OUTPUT_BYTES;
...
pub(crate) fn build_remote_tmux_command(config: &TmuxClientConfig) -> String {
    ...
    for arg in argv {
        let quoted = if previous_was_c {
            crate::discovery::shell_quote_remote_path(&arg)
        } else {
            crate::discovery::shell_quote(&arg)
        };
...
mod tests {
    ...
    impl TargetConnection for RecordingConnection {
        ...
        ) -> crate::transport::TransportResult<Box<dyn ByteChannel>> {
            ...
            for chunk_size in chunk_sizes {
                ...
                for chunk in bytes.chunks(chunk_size) {
                    for line in feed_bytes_to_lines(&mut buf, chunk) {
                        if let Some(crate::runtime::tmux::protocol::Message::Output {
                            content,
                            ..
