...
impl MockRuntime {
    ...
    /// Mutate the queued events from an external fixture crate.
    pub fn events_mut(&mut self) -> &mut Vec<StateChange> {
        &mut self.events
    }

    /// Mutate pane metadata from an external fixture crate.
    pub fn panes_mut(&mut self) -> &mut Vec<PaneInfo> {
        &mut self.panes
    }
}
