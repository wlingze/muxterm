...
pub use muxterm_transport::ssh::config::{list_ssh_hosts, parse_ssh_config, SshHostEntry};
...
pub fn list_local_tmux_sessions(socket: Option<&str>) -> Vec<TmuxSessionInfo> {
    let mut cmd = std::process::Command::new(crate::executable::resolve_tmux_binary());
...
mod tests {
    ...
    fn create_local_tmux_session_uses_detached_isolated_socket() {
        ...
        if create_local_tmux_session(Some(&socket), "proj", dir).is_err() {
            let _ = std::process::Command::new(crate::executable::resolve_tmux_binary())
        ...
        // 清理：只杀自己的测试 server。
        let cleanup = std::process::Command::new(crate::executable::resolve_tmux_binary())
            .args(["-L", &socket, "kill-server"])
            .output();
        assert!(
